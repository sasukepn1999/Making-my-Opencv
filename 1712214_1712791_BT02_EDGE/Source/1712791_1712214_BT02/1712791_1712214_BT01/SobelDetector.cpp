...
bool SobelDetector::LoadImage(const char* filename)
{
...
void SobelDetector::GaussianBlured(float sigma)
{
...
int SobelDetector::Convolution(int a[3][3], int b[3][3])
{
	int res = 0;
	for (int i = 0; i < 3; i++)
		for (int j = 0; j < 3; j++) {
...
void SobelDetector::detectBySobel()
{
	...
	GaussianBlured(0.8);
	sobelImg = Mat::zeros(grayImg.size(), grayImg.type());
	sobelX = Mat::zeros(grayImg.size(), grayImg.type());
	sobelY = Mat::zeros(grayImg.size(), grayImg.type());
	...
	for (int x = 1; x < height - 1; x++) {
		for (int y = 1; y < width - 1; y++) {
			...
			sobelImg.at<uchar>(x, y) = saturate_cast<uchar>(sqrt(X * X + Y * Y));
			sobelX.at<uchar>(x, y) = saturate_cast<uchar>(X);
			sobelY.at<uchar>(x, y) = saturate_cast<uchar>(Y);
		}
	}
}

bool SobelDetector::ShowImage()
{
	if (!grayImg.data)
	{
	...
	imshow("My Sobel", sobelImg);
	imshow("My Sobel X", sobelX);
	imshow("My Sobel Y", sobelY);
	return true;
}